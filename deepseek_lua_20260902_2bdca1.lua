-- =========================================================
-- BONK HUB | EGG FARM + JUMP SYSTEM (FULL)
-- =========================================================
...
-- =========================================================
-- HUMANIOID REPLACE (จาก Jump System)
-- =========================================================
...
-- =========================================================
-- HUMANOID BYPASS (ใช้ ReplaceHumanoid)
-- =========================================================
...
local function ApplyHumanoidBypass()
	...
	return ReplaceHumanoid(character)
end
...
local function TweenTo(targetPosition, speedOverride)
	...
	humanoid = character:FindFirstChild("BypassedHumanoid") or character:FindFirstChildOfClass("Humanoid")
	...
	connection = RunService.Heartbeat:Connect(function()
		...
		local currentHumanoid = currentCharacter:FindFirstChild("BypassedHumanoid") or currentCharacter:FindFirstChildOfClass("Humanoid")
		...
		if distance > 0.05 then
			currentRoot.CFrame = CFrame.new(
				currentPos,
				Vector3.new(targetPosition.X, currentPos.Y, targetPosition.Z)
			)
		end
...
-- =========================================================
-- EGG SESSION (แก้ไขแล้ว: ตรวจจับไข่หลุด)
-- =========================================================
...
local function RunEggSession(uid)
	...
	task.spawn(function()
		while sessionActive do
			...
			-- ✅ ถ้าไข่หลุดจากมือ ให้กลับไปเอาใหม่
			if not IsCarryingUid(uid) and EggRecordExists(uid) then
				local egg = GetEggByUid(uid)
				if egg then
	...
	if ActiveSession then
		...
		if currentUid then
			...
			if not target then
				...
				-- ✅ ถ้าไม่ได้ถือไข่ ให้หยุดเซสชั่นแล้วไปเอาใหม่
				if not IsCarryingUid(currentUid) then
					ActiveSession.Stop()
					ActiveSession = nil
					task.wait(0.3)
				else
...
-- =========================================================
-- CARRY CHANGED (ตรวจจับไข่หลุด)
-- =========================================================

EggState.CarryChanged:Connect(function(data)
	lastCarryData = data

	-- ✅ ถ้าไข่หลุดจากมือ ให้หยุดเซสชั่น
	if ActiveSession and data == nil then
		ActiveSession.Stop()
		ActiveSession = nil
	end
end)
...
-- Character Lifecycle (สำหรับ Jump)
...
if LocalPlayer.Character then
	task.spawn(function()
		ReplaceHumanoid(LocalPlayer.Character)
	end)
end
