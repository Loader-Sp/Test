-- =========================================================
-- BONK HUB | EGG FARM + JUMP SYSTEM (FIXED)
-- =========================================================
...
local function ReplaceHumanoid(character)
	...
	local rootPart = old.RootPart or character:FindFirstChild("HumanoidRootPart")
	...
	new.Health = math.min(saved.Health or 100, saved.MaxHealth or 100)
	CurrentHumanoid = new
...
local function UnlockCharacter(humanoid, root)
	...
	for _, obj in ipairs(root:GetChildren()) do
		if obj:IsA("BodyPosition") or obj:IsA("BodyGyro") or obj:IsA("AlignPosition")
			or obj:IsA("AlignOrientation") or obj:IsA("BodyVelocity") or obj:IsA("BodyThrust")
	...
	for _, obj in ipairs(root:GetChildren()) do
		if obj:IsA("Weld") or obj:IsA("Motor") then
			if obj.Part1 and (obj.Part1:IsA("Seat") or obj.Part1:IsA("VehicleSeat")) then
				obj:Destroy()
			end
		end
	end
...
local function DoJump()
	...
	if humanoid.UseJumpPower then
		power = humanoid.JumpPower > 0 and humanoid.JumpPower or 50
	else
		local height = humanoid.JumpHeight > 0 and humanoid.JumpHeight or 7.2
		power = math.sqrt(2 * Workspace.Gravity * height)
	end
...
UIS.JumpRequest:Connect(function()
	JumpHeld = true
	if not Jumping then DoJump() end
end)
...
task.spawn(function()
	if Player:FindFirstChild("PlayerGui") then
		...
		Player.ChildAdded:Connect(function(child)
			if child.Name == "PlayerGui" then SetupMobileJump() end
		end)
	end
end)
...
local function TweenTo(targetPosition, speedOverride)
	...
	if CurrentHumanoid == nil or CurrentHumanoid.Parent ~= character then
	...
	connection = RunService.Heartbeat:Connect(function()
		...
		if not currentRoot or not currentRoot.Parent or not currentHumanoid or currentHumanoid.Health <= 0 then
			stopped = true
			if connection then connection:Disconnect() connection = nil end
			return
		end
		...
		if distance <= ARRIVE_DISTANCE then
			if currentBodyVelocity and currentBodyVelocity.Parent == currentRoot then
		...
		if distance > 0.05 then
			currentRoot.CFrame = CFrame.new(currentPos, Vector3.new(targetPosition.X, currentPos.Y, targetPosition.Z))
		end

		local calculatedSpeed = math.min(speed, math.max(30, distance * 8))
		if currentBodyVelocity and currentBodyVelocity.Parent == currentRoot then
			currentBodyVelocity.Velocity = offset.Unit * calculatedSpeed
		end
	end)

	while not reached and not stopped and AutoFarm do
		local currentCharacter = GetCharacter()
	...
	if connection then connection:Disconnect() connection = nil end
	if currentBodyVelocity and currentBodyVelocity.Parent == root then
		currentBodyVelocity.Velocity = Vector3.zero
	end
	return reached
end
...
local function IsCarryingUid(uid)
	return uid and lastCarryData and tostring(lastCarryData.Uid) == tostring(uid)
end
...
local function GetEggFilterData()
	...
	if not fieldData or not fieldData.Records then return zones, names end
	for _, record in ipairs(fieldData.Records) do
		...
		if not zoneSet[zone] then zoneSet[zone] = true table.insert(zones, zone) end
		if not nameSet[name] then nameSet[name] = true table.insert(names, name) end
	end
	table.sort(zones)
	table.sort(names)
	return zones, names
...
local function GetFilteredEggs()
	local fieldData = EggState.ReadFieldEggs()
	if not fieldData or not fieldData.Records then return {} end
	local result = {}
	for _, record in ipairs(fieldData.Records) do
		local zone = tostring(record.AreaId or "Unknown")
		local name = tostring(record.AssetCategory or "Unknown")
		local mutation = GetEggMutation(record)
		if IsSelected(SelectedEggZones, zone) and IsSelected(SelectedEggNames, name) and IsSelected(SelectedMutations, mutation) then
...
local function GetNearestFilteredEgg()
	...
	local nearestPart, nearestUid, nearestDistance = nil, nil, math.huge
...
task.spawn(function()
	while task.wait(0.5) do
		...
		if StatusParagraph then
			...
			StatusParagraph:SetDesc("quantity Eggs: " .. tostring(#filtered) .. "\n" .. statusText)
		end
	end
end)
...
local function GetCurrentTarget(uid)
	...
	if not EggRecordExists(uid) then return nil end
	return target
end
...
local function RecoverTarget(uid)
	...
	local targetPosition = target.Position + Vector3.new(0, 2, 0)
...
local function RunEggSession(uid)
	if not uid or Lll then return false end
	local root = GetRoot()
	local humanoid = GetHumanoid()
	if not root or not humanoid or humanoid.Health <= 0 then return false end
	...
	task.spawn(function()
		while sessionActive do
			...
			if not EggRecordExists(uid) then
				if recoveryAttempts < maxRecoveryAttempts then
					recoveryAttempts = recoveryAttempts + 1
					task.wait(0.5)
					if not RecoverTarget(uid) then
						sessionActive = false
						break
					end
				else
	...
	task.spawn(function()
		...
		while sessionActive do
			...
			if IsCarryingUid(uid) then
				...
				local egg = GetEggByUid(uid)
				if egg then
					targetPos = egg.Position + Vector3.new(0, 2, 0)
				else
					targetPos = nil
				end
			end
...
local function SetupCharacter(character)
	...
	if root then SpawnPosition = root.Position end
	...
	if AutoFarm then
		...
		task.delay(1.5, function()
			...
			if not currentHumanoid or currentHumanoid.Health <= 0 or not currentRoot then return end
			StartFarmLoop()
		end)
	end
end
...
Player.CharacterAdded:Connect(function(character)
	ResetStateOnDeath()
	if AutoFarm then NeedInitialDropOff = true end
...
local PlayerGroup = PlayerPage:Groupbox("Anti-Ragdoll", "Left")
...
local function Fix(Character)
	...
	if not Humanoid.PlatformStand and Humanoid:GetState() ~= Enum.HumanoidStateType.Physics then return end
...
PlayerGroup:AddToggle({
	...
	Callback = function(Value)
		...
		if Value then AntiKnockback() end
	end
})